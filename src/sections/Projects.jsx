      ...
      <div className="w-full text-white">
        ...
        <div className="grid gap-6 sm:grid-cols-2 mt-6">
          {serverLinks.map((project) => (
            <article key={project.id} className="rounded-lg bg-gray-900 border border-zinc-700 shadow-lg overflow-hidden group transition-transform hover:scale-[1.02]">
              <div className="h-full flex flex-col sm:flex-row">
                <div className="sm:w-1/3 bg-linear-to-br from-gray-800 to-gray-700 flex items-center justify-center p-4">
                  {project.image ? (
                    <img
                  ...
                  <div>
                    ...
                    <ul className="flex flex-wrap gap-2 mt-4">
                      {project.stack.map((tech, idx) => (
                        <li key={idx} className="text-xs px-2 py-1 rounded-full bg-gray-800 text-zinc-200 border border-zinc-700">{tech}</li>
                      ))}
                    </ul>
                  </div>
                  ...
                  <div className="mt-4 flex gap-3">
                    {project.link && project.link.trim() !== "" && (
                      <a
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-gray-800 hover:bg-gray-700 text-white text-sm font-medium border border-zinc-700"
